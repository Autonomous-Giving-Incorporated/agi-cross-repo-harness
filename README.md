...
5. Add allocation, optional dual-approval, delegation, evidence, and public-projection acceptance tests. **Started:** allocation lifecycle ordering + single/dual approval landed in `src/lifecycle.mjs`; downstream least-context delegation hand-off landed in `src/downstream-handoff.mjs`. Remaining: evidence-attachment acceptance, which needs a synthetic evidence-record contract to be defined first.
